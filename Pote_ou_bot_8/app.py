...
def get_last_message():
    if messages:
        if messages[-1]['content'] != "Premier arrivé (devineur).":
            return messages[-1]
...
def check_new_messages():
    if 'last_message' not in session:
        session['last_message'] = {'author': "", 'content': ""}

    # Récupère le dernier message
    new_last_message = get_last_message()

    # Si nouveau message de l'autre
    if (session['last_message'] != new_last_message) and (new_last_message['author'] != session['username']) and (new_last_message['author'] != "Système"):
        session['last_message'] = new_last_message
        return jsonify({'newMessagesReceived': True, 'lastMessage': new_last_message})
    else:
